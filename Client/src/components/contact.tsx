  ...
  return (
    <div className='bg-detailSectionBgNav/75 h-fit lg:h-full w-full text-gray-300' >
      <div className=' flex gap-5 flex-col w-full items-center lg:my-20  my-36 '>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className='mx-8  text-gray-300'>
          <div className='flex w-full flex-col md:flex-row gap-4 mb-4'>
            
            <input type="text" placeholder='Full Name' className={`${inputStyle}  text-gray-300 md:w-1/2 focus:border-[1px] border-[0px] shadow-md font-[400] bg-[#323232] ${errors.fullName && " border-red-700 border-[3px]"}`} {...register("fullName", {
              required: "Full name is required"
            })} />
            
            <input type='email' placeholder='Email' className={`${inputStyle} text-gray-300 md:w-1/2 border-[0px] shadow-md font-[400] bg-[#323232] focus:border-[1px] ${errors.email && " border-red-700 border-[3px]"}`} {...register("email", {
          ...
          <button type='submit' disabled={isSubmitting} className={`${inputStyle} bg-secondary_tx_color  w-full py-3 px-4 font-bold `}>Send Mail</button>
        </form>
