...
const aboutmeEdit = asyncHandler(async (req, res) => {
  ...
  const aboutmesaveResp = await Aboutme.create({
    titleHeader,
    titleDesp,
    numberOfProjects,
    yearsOfExp,
...
const getAboutmeContent = asyncHandler(async (req, res) => {
  ...
  await Aboutme.findOne({})
    .then((user) => {
      return res
        .status(201)
        .json(new ApiResponse(200, user, "Return aboutMe from the dbs"));
    })
    .catch((err) => {
      return res
        .status(501)
        .json(new ApiError(501, "Error return the aboutMeData", err));
    });
});
    ...
    .then((user) => {
      return res
        ...
        .json(
          new ApiResponse(
            201,
            user.specializedFields,
            "Return specializedFields from dbs"
          )
        );
    })
    .catch((err) => {
      return res
        .status(501)
        .json(new ApiError(501, "Error return the specialized data", err));
    });
});
